...
- LayoutLMv3将文本、图像和版面位置联合建模，说明教学文档中的空间结构具有语义价值；
- Donut指出传统OCR可能产生计算开销、语言适配困难和误差级联；
- ColPali直接嵌入文档页面图像，通过多向量后期交互完成检索，避免完全依赖脆弱的OCR流水线。
...
CLIP证明可以使用对比学习把图像和自然语言映射到共享空间；BLIP-2通过轻量Q-Former连接冻结视觉编码器和语言模型；ImageBind进一步把文本、图像、音频等六种模态绑定到统一空间。
...
Whisper表明大规模弱监督可以获得较强的多语种、跨噪声语音识别能力；ImageBind提供音频、视频与文本对齐思路。
...
普通固定长度切片容易割裂“教学目标—知识点—例题—答案—解析”的关系。建议采用教学结构感知分块：
...
DPR显示稠密双编码器可以显著提升开放域问答召回，但相关研究也表明稠密检索对长尾实体并非总是可靠，因此应保留BM25。最终使用RRF或加权融合合并结果。
...
1. 各通道快速召回Top-N；
2. 权限、课程、学段、学科和资源版本过滤；
3. 使用交叉编码器或多向量后期交互重排；
4. 对高度重复的教材版本和课件页去重；
5. 选出覆盖不同子问题的证据，而不是只保留相似度最高的一组近重复文本。

ColBERT用token级表示和MaxSim后期交互兼顾效率与细粒度相关性；ColPali把这一思想扩展到文档图像块。项目可将“初召回便宜、精排昂贵”作为分层设计原则。

#### 3.2.3 查询改写与教师意图展开

HyDE先生成一个假设性文档，再用其向量寻找真实文档，适合将教师口语化需求转换为教材表达。但假设文档本身可能包含幻觉，因此只能用于检索，不应作为最终事实来源。
...
RAG将参数记忆与可更新的外部非参数记忆结合；MuRAG进一步从图像和文本组成的多模态记忆中检索证据；RA-CM3说明检索到的多模态示例也可增强多模态生成。
...
Self-RAG、DRAGIN和自适应多模态RAG共同说明：并非每个请求都需要检索，也不应为所有问题固定检索相同数量的文档。
...
Lost in the Middle证明，即便模型支持长上下文，关键信息位于上下文中部时利用效果也可能下降。因此不能把大量召回结果直接塞入提示词。
...
CRAG将检索结果判断为`Correct / Ambiguous / Incorrect`，再触发不同动作。项目可以实现三级证据门控：
...
CRAG强调完整文档中常包含大量无关内容。项目应把召回页面进一步分解为知识单元，过滤无关信息后再重组上下文：
...
Self-RAG利用反思标记判断是否检索、证据是否相关、回答是否被支持以及答案质量。项目不一定需要重新训练Self-RAG模型，可以把思想实现为显式工作流：
...
CRITIC说明模型可以借助外部工具对初稿进行批判和修正。教学项目可按内容类型调用不同核验工具：
...
不可回答性评测研究指出，RAG系统不仅要回答有答案的问题，还应识别知识库中没有答案的问题。项目应提供三种安全输出：
...
对于PPT和Word导出，可在教师备注、脚注或附录中保留来源。结构化输出研究表明，RAG也适合约束工作流、JSON或教案结构的生成，但仍需字段级校验。
...
RAGAS和ARES将评测拆成上下文相关性、答案忠实度和答案相关性。项目可扩展为：
...
| 编号 | 论文/技术 | 对项目的主要价值 |
|---|---|---|
| 01 | CLIP | 图文共享嵌入、文字搜图、以图搜资源 |
| 02 | LayoutLMv3 | 文本、图像和版面联合文档理解 |
| 03 | Donut | 无OCR文档理解及OCR误差级联分析 |
| 04 | Whisper | 课堂录音和教学视频转写 |
| 05 | ImageBind | 文本、图像、音频等统一向量空间 |
| 06 | BLIP-2 | 低成本连接视觉编码器与语言模型 |
| 07 | ColPali | 教材/PPT页面原生视觉多向量检索 |
| 08 | RAG | 参数记忆与可更新外部知识结合 |
| 09 | DPR | 稠密双编码器语义召回 |
| 10 | ColBERT | token级后期交互和精细重排 |
| 11 | HyDE | 模糊教学需求的假设文档查询扩展 |
| 12 | MuRAG | 同时检索图像和文本证据 |
| 13 | RA-CM3 | 用外部多模态记忆增强多模态生成 |
| 14 | MRAG综述 | 多模态RAG组件、数据集与评测全景 |
| 15 | Lost in the Middle | 重排、压缩和证据位置的重要性 |
| 16 | Self-RAG | 按需检索、证据评价和生成自我反思 |
| 17 | CRAG | 检索质量分级、纠错检索和内容过滤 |
| 18 | CRITIC | 使用外部工具核验并修正生成结果 |
| 19 | 结构化输出幻觉抑制 | 教案、任务计划和结构化成果的RAG约束 |
| 20 | RAGAS | 上下文相关性、忠实度、答案相关性评测 |
| 21 | ARES | 少量人工标注下的自动化RAG评测 |
| 22 | 不可回答性评测 | 无证据问题识别、拒答和澄清 |
| 23 | 检索增强趋势综述 | 检索时机、策略和结果利用的中文框架 |
| 24 | 多模态RAG综合综述 | 查询、检索、融合、增强、生成分类 |
| 25 | M2RAG | 多模态输入到多模态输出及评价基线 |
| 26 | 自适应多模态RAG | 动态控制检索数量、降低噪声 |
| 27 | DRAGIN | 根据模型信息需求决定何时检索、检索什么 |
