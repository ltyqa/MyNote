...
本笔记基于项目论文库中的34份 PDF 整理，共约764页；其中 P24 是 arXiv 预印本，P28 是同一工作的 Findings of ACL 2025 正式版，因此对应33项独立研究工作。正文统一引用正式版编号 P28，P24 仅作为版本记录保留。新增精读覆盖：
...
Ask in Any Modality 的 ACL 2025 正式版本从检索、融合、增强、生成和训练五个顶层维度组织多模态 RAG 方法，并在生成部分进一步讨论智能体式生成与交互；论文还将精确跨模态归因、统一嵌入、组合推理、知识投毒防御及成本可扩展性列为开放问题。[P28] 本项目的核心在线流程是“双索引—动态路由—逻辑检索—声明核验”；教学结构感知分块属于离线知识组织策略，权威知识与教师私有知识融合属于贯穿检索和生成的数据源治理策略。各模块的有效性仍须分别通过中文教学数据验证。
...
说明：P24 与 P28 是同一项研究。P24 为 arXiv 预印本，P28 为 Findings of ACL 2025 正式发表版本；正文统一引用 P28，P24 仅用于记录版本沿革，不重复视为独立方法。
...
| P28 | Ask in Any Modality（Findings of ACL 2025正式版） | 从检索、融合、增强、生成和训练五个顶层维度组织多模态RAG方法，并讨论智能体式生成、评测和开放问题；正文统一引用此编号 |
